...
function getFontFiles() {
    ...
    try { return fs.readdirSync(dir).filter(f => f.endsWith('.ttf')).map(f => path.join(dir, f)); }
    catch { return []; }
}

const _fontFiles = getFontFiles();
...
function svgToPng(svgString) {
    const resvg = new Resvg(svgString, {
        font: {
            loadSystemFonts: _fontFiles.length === 0,
...
async function pngToWebp(pngBuf) {
...
module.exports = { applyEffects, textToSticker, createQuoteCard, escapeXml, FONT_PATH, FONT_ARIAL, imgToWebp, rawToWebp, svgToPng, svgToSticker, pngToWebp, getFontFiles };
