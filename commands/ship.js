...
module.exports = async (client, msg) => {
    ...
    try {
        ...
        const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="512" height="512">
            ...
            <text x="256" y="170" text-anchor="middle" font-family="serif" font-size="80" fill="white">💘</text>
            <text x="256" y="280" text-anchor="middle" font-family="Impact, Arial, sans-serif" font-size="80" fill="white" paint-order="stroke" stroke="black" stroke-width="3">${percentage}%</text>
            <text x="256" y="340" text-anchor="middle" font-family="Arial, sans-serif" font-size="30" fill="white">${escapeXml(verdict)}</text>
            <text x="256" y="410" text-anchor="middle" font-family="Arial, sans-serif" font-size="28" fill="white">${heartStr}</text>
        </svg>`;
        const pngBuf = svgToPng(svg);
        const media = new MessageMedia('image/png', pngBuf.toString('base64'));
        await msg.reply(media, undefined, { caption: `💕 *SHIP CALCULATOR*\n\n💕 ${verdict}\nMatch: ${percentage}%` });
        await react(msg, '✅');
    } catch (e) {
