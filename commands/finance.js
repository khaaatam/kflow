const { MessageMedia } = require('whatsapp-web.js'); // 👈 WAJIB ADA!
...
module.exports = async (client, msg, args, senderId, namaPengirim, text) => {
    ...
    // --- 1. FITUR CATAT ---
    if (command === '!catat') {
        // ... (Kode !catat lu yang udah bener tadi, biarin aja) ...
        // (Copy paste dari file sebelumnya, gak ada perubahan di sini)
        ...
        if (args[1] && ['pemasukan', 'pengeluaran'].includes(args[1].toLowerCase()) && !isNaN(parseInt(args[2]))) {
            ...
            await msg.react('🧠');
            try {
                const prompt = `Role: Finance Assistant. Tugas: Ekstrak data keuangan dari: "${rawText}". Output JSON lowercase: {"jenis": "...", "nominal": 0, "keterangan": "..."}`;
                ...
                return msg.reply("❌ Gagal paham. Manual aja.");
            }
        }
