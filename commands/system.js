...
module.exports = async (client, msg, text, senderId, namaPengirim) => {
    ...
    if (text === '!help' || text === '!menu') {
        ...
        // 2. FUNGSI PENEBAK ICON (Biar Dinamis) 🧠
        const getSmartIcon = (categoryName) => {
            ...
            if (cat.includes('GAME') || cat.includes('FUN')) return '🎮';
            if (cat.includes('TOOL') || cat.includes('UTIL')) return '🛠️';
            if (cat.includes('INFO') || cat.includes('NEWS')) return 'ℹ️';
            if (cat.includes('STICKER') || cat.includes('STIKER')) return '🎨';
            if (cat.includes('ISLAM') || cat.includes('RELIGI')) return '🕌';
            if (cat.includes('ANIME') || cat.includes('MANGA')) return '⛩️';
            if (cat.includes('PREMIUM') || cat.includes('VIP')) return '💎';
        ...
        for (const cat in categories) {
            const icon = getSmartIcon(cat); // 👈 Disini dia nebak iconnya
            menuText += `${icon} *${cat}*\n${categories[cat].join('\n')}\n\n`;
        }
