...
// Observer rate limit: max 3 observations per minute per user
const observerLimiter = new RateLimiter(60000, 3);
...
const observe = async (client, msg, namaPengirim) => {
    ...
    // Rate limit: max 3 AI calls per minute per user
    if (!observerLimiter.check(msg.author || msg.from)) {
        logger.debug(`[OBSERVER] Rate limited for ${namaPengirim}`);
        return;
    }
